/*jshint esversion: 6 */
let questions = [{
    question: "Survivor had a hit with Eye of the Tiger. Which film did it feature in",
    option1: "Ghostbusters",
    option2: "Rocky III",
    option3: "Back to the Future II",
    option4: "Top Gun",
    answer: "2",
    },
    {
    question: "'When I look back on my life, it's always been with a sense of shame, I've always been the one to blame' is the opening lyric to which song?",
    option1: "'Bad' by Michael Jackson",
    option2: "'Buffalo Stance' by Neneh Cherry",
    option3: "'People are People' by Depeche Mode",
    option4: "'It's A Sin' by Pet Shop Boys",
    answer: "4",
    },
    
    {
    question: "Which of these is Not a Cyndi Lauper song?",
    option1: "True Colours",
    option2: "Girls Just Wanna Have Fun",
    option3: "Time After Time",
    option4: "Like a Prayer",
    answer: "4",
    },
    {
    question: "'I know when to go out, I know when to stay in, get things done' is the opening lyric to which song?",
    option1: "'She Blinded Me By Science' by Thomas Dolby",
    option2: "'Who Can It Be Now' by Men At Work",
    option3: "'Modern Love' by David Bowie",
    option4: "'Our House' by Madness",
    answer: "3"
    },
    
    {
    question: "Who sings 'Sunshine on Leith?",
    option1: "Queen",
    option2: "The Silencers",
    option3: "The Proclaimers",
    option4: "Big Country",
    answer: "3"
    },
    {
    question: "'I Believe the Children Are Our Future' is the opening lyric to which Whitney Houston song?",
    option1: "All At Once",
    option2: "One Moment in Time",
    option3: "The Greatest Love Of All",
    option4: "How Will I Know",
    answer: "3"
    },
    
    {
    question: "Which all-girl group had a Manic Monday?",
    option1: "Bananarama",
    option2: "The Bangles",
    option3: "En Vogue",
    option4: "Salt-N-Pepa",
    answer: "2"
    },
    {
    question: "'Desert loving in your eyes all the way' is the opening lyric to which song?",
    option1: "'Walk Like An Egyptian' by The Bangles",
    option2: "'Let's Dance' by David Bowie",
    option3: "'Karma Chameleon' by Culture Club",
    option4: "'Obsession' by Animotion",
    answer: "3"
    },
    
    {
    question: "How old were the George Michael and Andrew Ridgley when they wrote 'Careless Whisper'? ",
    option1: "17",
    option2: "18",
    option3: "19",
    option4: "20",
    answer: "1"
    },
    {
    question: "What year was Live Aid?",
    option1: "1984",
    option2: "1985",
    option3: "1986",
    option4: "1987",
    answer: "2",
    },
    {
    question: "'I must've dreamed a thousand dreams, been haunted by a million screams' is the opening lyric to which song?",
    option1: "'The Reflex' by Duran Duran",
    option2: "'Missing You' by John Waite",
    option3: "Land of Confusion' by Genesis",
    option4: "I Just Died in Your Arms Tonight' by Cutting Crew",
    answer: "3"
    },
    
    {
    question: "How many members were there in the Thompson Twins (1982 -1986)?",
    option1: "2",
    option2: "3",
    option3: "4",
    option4: "5",
    answer: "2"
    },
    {
    question: "What is Boy George's real name?",
    option1: "George Boyle",
    option2: "George Boysen",
    option3: "George O'Casey",
    option4: "George O'Dowd",
    answer: "4"
    },
    {
    question: "'I bought a toothbrush, some toothpaste, a flannel for my face, pajamas, a hairbrush, new shoes and a case' are the opening lyrics to which song?",
    option1: "'Got My Mind Set On You' by George Harrison",
    option2: "'Need You Tonight' by INXS",
    option3: "'Here I Go Again' by Whitesnake",
    option4: "'Labelled With Love' by Squeeze",
    answer: "4"
    },
    
    {
    question: "What was the name of Band Aid's charity single?",
    option1: "Feed the World",
    option2: "We are the World",
    option3: "Do They Know It's Christmas",
    option4: "It's Christmas Time",
    answer: "3"
    },
    {
    question: "What type of dance did Men Without Hats take to the charts in 1983?",
    option1: "safety",
    option2: "sack",
    option3: "jive",
    option4: "rumba",
    answer: "1"
    },
    {
    question: "What was the name of the Pixies first EP? ",
    option1: "Bossanova",
    option2: "Surfer Rosa",
    option3: "Come On Pilgrim",
    option4: "Doolittle",
    answer: "3"
    },
    
    {
    question: "Which Scandinavian Country do the band Aha come from?",
    option1: "Sweden",
    option2: "Norway",
    option3: "Finland",
    option4: "Denmark",
    answer: "2"
    },
    
    {
    question: "Kim Deal was known by what name when she joined the Pixies?",
    option1: "Mrs Peter Murphy",
    option2: "Mrs John Murphy",
    option3: "Mrs Jonathan Williams",
    option4: "Mrs Black Francis",
    answer: "2"
    },
    
    {
    question: "What was the first single released from Michael Jackson's 'Thriller' album?",
    option1: "Wanna Be Startin' Somethin'",
    option2: "Beat It",
    option3: "The Girl is Mine",
    option4: "Human Nature",
    answer: "3",
    },
    {
    question: "Which Pixies song appeared in the movie Fight Club?",
    option1: "Wave of Mutilation",
    option2: "Levitate Me",
    option3: "I Bleed",
    option4: "Where is my Mind",
    answer: "4"
    },
    
    {
    question: "Which Prince album featured the 1983 hit Little Red Corvette?",
    option1: "1999",
    option2: "Controversy",
    option3: "Dirty Mind",
    option4: "Purple Rain",
    answer: "1"
    },
    {
    question: "Which song features the line 'Some people call it a one night stand but we can call it paradise'?",
    option1: "The Human League 'Don't you want me Baby'",
    option2: "Spandau Ballet 'Gold'",
    option3: "Duran Duran 'Save A Prayer'",
    option4: "Madonna 'True Blue'",
    answer: "3"
    },
    {
    question: "'Hey little sister what have you done?' is the opening line to which song?",
    option1: "'One Thing Leads to Another' by the Fixx",
    option2: "'Cold Hearted Snake' by Paula Abdul",
    option3: "'Modern Love' by David Bowie",
    option4: "Billy Idol 'White Wedding'",
    answer: "4"
    },
    {
    question: "What was the biggest selling single of the 1980s in the UK?",
    option1: "'Do they know it's Christmas'",
    option2: "'Let's Dance' ",
    option3: "'Relax' ",
    option4: "'Come on Eileen'",
    answer: "1"
    },
    {
    question: "Madness had 15 top ten hits in the 1980s but only one number one. What was it?",
    option1: "Wings of a Dove",
    option2: "Baggy Trousers",
    option3: "It Must Be Love",
    option4: "House of Fun",
    answer: "House of Fun",
    },
    {
    question: "Which Duran Duran song opens with laughter from the keyboardist's girlfriend?",
    option1: "Girls on Film",
    option2: "The Reflex",
    option3: "Hungry Like The Wolf",
    option4: "Save A Prayer",
    answer: "3"
    },
    {
    question: "Which song gave Whitney Houston her first number one?",
    option1: "I Wanna Dance With Somebody",
    option2: "How Will I Know",
    option3: "Saving All My Love For You",
    option4: "Greatest Love Of All",
    answer: "3"
    },
    {
    question: "Three members of Duran Duran share the same surname - what is it? ",
    option1: "Smith",
    option2: "Taylor",
    option3: "Williams",
    option4: "Johnson",
    answer: "2"
    },
    {
    question: "Who said 'If Morissey says not to eat meat, then I'm going to eat meat: that's how much I hate Morissey'?",
    option1: "Robert Smith of The Cure",
    option2: "Julian Cope of the Teardrop Explodes",
    option3: "Michael Stipe of REM",
    option4: "Bono of U2",
    answer: "1"
    },
    {
    question: "'Tainted Love' was a hit for Soft Cell in 1981 but who recorded the original?",
    option1: "Martha and the Vandellas",
    option2: "The Supremes",
    option3: "Alison Moyet",
    option4: "Gloria Jones",
    answer: "4"
    },
    {
    question: "How many studio albums did REM release in the 80s?",
    option1: "4",
    option2: "5",
    option3: "6",
    option4: "7",
    answer: "3",
    },
    {
    question: "Which of the following movies did NOT feature David Bowie?",
    option1: "Jazzin' for Blue Jean",
    option2: "The Hunger",
    option3: "Velvet Goldmine",
    option4: "The Last Temptation of Christ",
    answer: "3"
    },
    {
    question: "'Give me time to realise my crime' is the opening line to which song?",
    option1: "'Time' by the Alan Parsons Project",
    option2: "'Time After Time' by Cindi Lauper",
    option3: "'Do You Really Want to Hurt Me' by Culture Club",
    option4: "'Video Killed the Radio Star' by the Buggles",
    answer: "3"
    },
    {
    question: "'That's great! It starts with an earthquake, birds and snakes an aeroplane, Lenny Bruce is not afraid' are the opening lyrics to which song?",
    option1: "'We Didn't Start the Fire' by Billy Joel",
    option2: "'Everybody Wants to Rule the World' Tears for Fears",
    option3: "'You Dropped a Bomb On Me' by The Gap Band",
    option4: "'It's the End of the World As We Know It' by REM",
    answer: "4"
    },
    {
    question: "'Show me show me show me how you do that trick, the one that makes me scream she said', are the opening lines of which song?",
    option1: "'Hungry like the Wolf' by Duran Duran?",
    option2: "'Just Like Heaven' by the Cure",
    option3: "'Sign Of the Times' by Prince ",
    option4: "'See you' by Depeche Mode",
    answer: "2"
    },
    {
    question: "Which singer died in 1997?",
    option1: "John Lennon",
    option2: "Billy Mackenzie",
    option3: "Kurt Cobain",
    option4: "Ian Curtis",
    answer: "2"
    },
   
];